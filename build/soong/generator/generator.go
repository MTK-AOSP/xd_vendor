...
func init() {
	android.RegisterModuleType("xd_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = xdExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = xdExpandVariables(ctx, dep_file)
	...
	cmd := xdExpandVariables(ctx, String(g.properties.Cmd))
